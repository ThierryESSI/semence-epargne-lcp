...
const TABS = ['Toutes','Disponibles','Attribuées','Utilisées'];

export default function Cartes() {
  const [tab, setTab] = useState('');
  const { items, pagination, loading, page, setPage, search, setSearch, refetch } = usePaginated<any>('/cartes/emettre', tab && tab !== 'Toutes' ? { statut: tab.toUpperCase().replace('É','E').replace('ES','E') } : {});
  ...
  async function handleEmettre(e:any) {
    ...
    try {
      const {data} = await api.post('/cartes/emettre', { montant, quantite });
      setEmises(data.data || []); refetch();
  ...
  return (
    <div>
      <PageHeader title="Cartes Semence Épargne" subtitle="Gestion et émission des cartes prépayées"
        action={<>
      ...
      <div style={{display:'flex',gap:0,marginBottom:16,alignItems:'center'}}>
        {TABS.map((t,i)=>(
          <button key={t} onClick={()=>setTab(t==='Toutes'?'':t)}
      ...
      <TableWrapper>
        <THead cols={['Code carte','Montant','Frais (1%)','Part LCP','Part Distrib.','Crédité','Code valid.','Statut','Date']}/>
        <tbody>
          {loading ? <tr><td colSpan={9}><Spinner/></td></tr>
          : items.length===0 ? <tr><td colSpan={9}><Empty msg="Aucune carte. Émettez votre premier lot."/></td></tr>
          : items.map((c:any,i:number)=>{
            const mnt=Number(c.montant||0);
            const frais=Math.ceil(mnt*0.01);
            return (
              <TR key={c.id}>
                <TD><span style={{fontFamily:'monospace',color:C.green,fontSize:12,fontWeight:600}}>{c.reference}</span></TD>
      ...
      {/* Modal émission */}
      {showEmission && (
        <Modal title="Émission de cartes" onClose={()=>{setShowEmission(false);setEmises([]);setError('');}}>
          {emises.length > 0 ? (
            <div>
              ...
              <Btn variant="secondary" onClick={()=>{setShowEmission(false);setEmises([]);}} style={{width:'100%',justifyContent:'center',marginTop:12}}>Fermer</Btn>
            </div>
          ) : (
