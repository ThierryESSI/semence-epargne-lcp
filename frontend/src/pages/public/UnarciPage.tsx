...
export default function UnarciPage() {
  ...
  // Après paiement : ouvre WhatsApp avec le message pré-rempli + preuve à joindre
  function notifierPaiement() {
    if (!result) return;
    const msg = `Bonjour, je viens de payer ma cotisation UNARCI.\nRéférence : ${result.reference}\nCompte : ${result.numeroCompte}\nMontant : ${result.montant.toLocaleString('fr-CI')} FCFA\nVoici ma preuve de paiement (capture ci-jointe) :`;
      ...
      <div style={{ maxWidth:760, margin:'-80px auto 60px', padding:'0 6%' }}>
        {result ? (
          <div style={{ background:WHITE, borderRadius:20, border:`1px solid ${BORDER}`, boxShadow:'0 8px 40px rgba(15,46,82,0.12)', padding:'40px 32px', textAlign:'center' }}>
            ...
            <button type="button" onClick={notifierPaiement}
              style={{ display:'inline-flex', alignItems:'center', gap:8, marginTop:16, background:'#25D366', color:'#fff', border:'none', borderRadius:10, padding:'12px 22px', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit', boxShadow:'0 4px 16px rgba(37,211,102,0.4)' }}>
            ...
            <Link to="/" style={{ display:'block', marginTop:16, color:SEC, fontSize:13, fontWeight:600, textDecoration:'none' }}>Retour à l'accueil</Link>
          </div>
        ) : (
